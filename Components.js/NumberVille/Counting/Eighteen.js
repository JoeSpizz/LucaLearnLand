...
function Eighteen(props) {  const [pressed, setPressed] = useState([]);
    const [counters, setCounters] = useState(Array(18).fill(null));
    ...
    require(`../../../assets/numbers/16.mp3`),
    require(`../../../assets/numbers/17.mp3`)
  ]
const handlePress = (i)=>{
if (!counters[i]) {
  ...
  if((tally+1)===18){
    async function countLast() {
      const { sound, status } = await Audio.Sound.createAsync(
        require('../../../assets/numbers/18s.mp3')
      );
        ...
        <View key={i} >
          <TouchableWithoutFeedback onPressIn={pressed.includes(i) ? null: ()=>handlePress(i) }>
           <Text style={styles.imageContainer}>
            <View>
                <Image style={pressed.includes(i) ? styles.buttonPressed : styles.picture} source={require('../../../assets/countgame/motorcycle.png')}/>  
            </View>
           </Text>
